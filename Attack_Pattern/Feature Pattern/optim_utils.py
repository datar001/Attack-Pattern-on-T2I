...
def optimize_prompt_loop(model, tokenizer, all_target_features,
    ...
    for step in range(opt_iters):
        ...
        # save top k prompt
        if cosim_scores >= thres and decoded_text not in save_prompts:
            prompt_topk.append([cosim_scores, mse_loss.item(), decoded_text, decoded_token])
